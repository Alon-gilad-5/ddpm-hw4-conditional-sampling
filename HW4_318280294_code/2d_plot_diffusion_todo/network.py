...
class SimpleNet(nn.Module):
    def __init__(
        ...
        self.num_timesteps = num_timesteps
        self.fc1 = TimeLinear(dim_in, dim_hids[0], num_timesteps)
        self.hidden_layers = nn.ModuleList()
        for i in range(len(dim_hids) - 1):
            self.hidden_layers.append(TimeLinear(dim_hids[i], dim_hids[i + 1], num_timesteps))
        self.fc_out = nn.Linear(dim_hids[-1], dim_out)
        self.act = nn.SiLU()
        ######################
