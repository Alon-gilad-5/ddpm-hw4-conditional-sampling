...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        #forward process: xt = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * noise
    ...
    def p_sample(self, xt, t):
        ...
        betas_at_t = extract(self.var_scheduler.betas, t, xt)
        alphas_at_t = extract(self.var_scheduler.alphas, t , xt)
        alphas_prod_at_t = extract(self.var_scheduler.alphas_cumprod, t, xt)

        #calc mean of p(x_t-1 | x_t)
        mean = (xt - eps_factor * eps_theta) / torch.sqrt(alphas_at_t)
        #adding noise
        if t > 0:
            noise = torch.randn_like(xt)
            std = torch.sqrt(betas_at_t)
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        #direction to xt
        pred_dir = torch.sqrt(1 - alpha_prod_t_prev - sigma_t ** 2) * eps_theta

        noise = torch.randn_like(xt) if eta > 0 else 0
        x_t_prev = torch.sqrt(alpha_prod_t_prev) * pred_x0 + pred_dir + sigma_t * noise
        ######################
        return x_t_prev
