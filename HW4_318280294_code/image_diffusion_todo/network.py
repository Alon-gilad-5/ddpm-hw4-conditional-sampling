    ...
    def forward(self, x, timestep, class_label=None):
        ...
        if self.use_cfg and class_label is not None:
            if self.training:
                ...
                #when dropped, set to the null class (num_classes)
                batch_size = class_label.shape[0]
                drop_mask = torch.rand(batch_size, device=class_label.device) < self.cfg_dropout

                #the null class is the last embedding index (num_classes)
                null_class = torch.full_like(class_label, self.class_embedding.num_embeddings - 1)
                class_label = torch.where(drop_mask, null_class, class_label)
                #######################
