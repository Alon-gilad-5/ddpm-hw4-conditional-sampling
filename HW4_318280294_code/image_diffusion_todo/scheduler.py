...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        #these are the params for timestep t
        alpha_at_t = self._get_teeth(self.alphas,t)
        alpha_cumprod_at_t = self._get_teeth(self.alphas_cumprod,t)
        betas_at_t = self._get_teeth(self.betas,t)
        sigma_at_t = self._get_teeth(self.sigmas,t)

        # DDPM reverse step formula from Algorithm 2:
        # x_t-1 = 1/sqrt(alpha_t) * (x_t - (1-alpha_t)/sqrt(1-alpha_bar_t) * eps_theta) + sigma_t * z

        mean = (1.0 / torch.sqrt(alpha_at_t)) * (x_t - (betas_at_t / torch.sqrt(1 - alpha_cumprod_at_t)) * eps_theta)
        if t > 0:
            noise = torch.randn_like(x_t)
            sample_prev = mean + sigma_at_t * noise
        else:
            sample_prev = mean
        #######################
    ...
    def add_noise(
        ...
        # DDPM forward process: x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * eps
