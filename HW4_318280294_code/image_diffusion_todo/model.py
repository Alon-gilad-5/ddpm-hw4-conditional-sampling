...
class DiffusionModule(nn.Module):
    ...
    def get_loss(self, x0, class_label=None, noise=None):
        ...
        x_t, noise = self.var_scheduler.add_noise(x0, timestep)  # noise is created inside
    ...
    def sample(
        ...
        if do_classifier_free_guidance:
            ...
            #null labels - last embedding index
            null_class_label = torch.full_like(class_label,self.network.class_embedding.num_embeddings - 1)

            #concat null labels and actual labels
            class_label = torch.cat([null_class_label, class_label], dim=0)
        ...
        for t in tqdm(self.var_scheduler.timesteps):
            ...
            if do_classifier_free_guidance:
                ######## TODO ########
                # Create a combined input for the network
                x_t_combined = torch.cat([x_t, x_t], dim=0)

                # Get the noise prediction for both conditional and unconditional inputs
                noise_pred_combined = self.network(
                    x_t_combined,  # Use the combined tensor here
                    timestep=t.to(self.device),
                    class_label=class_label
                )

                # Split the predictions into unconditional and conditional parts
                noise_pred_uncond, noise_pred_cond = noise_pred_combined.chunk(2, dim=0)

                # Combine the predictions using the guidance scale
